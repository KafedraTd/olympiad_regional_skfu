...
function getTransfering () {
    ...
    Array.from(btns).forEach(el => el.addEventListener('click', event => {
        ...
        if (el.getAttribute('work')=='ВСО РЕГ 23: ТЕСТЫ'){
            window.open('TS/testing_23_reg.html', '_blank');
        } else if (el.getAttribute('work')=='ВСО РЕГ 23: ЗАДАЧИ'){
            window.open('TS/tasking_23_reg.html', '_blank');
        }else if (el.getAttribute('work')=='ВСО РЕГ 23: ДЕКЛАРАЦИЯ'){
            window.open('DT/declarationForGoods0.html', '_blank');
        }else if(el.getAttribute('work')=='ВСО ВУЗ 23: ТЕСТЫ'){
